...
import loader
#from jedyBot import JedyBot
#import logging
#import datetime
#import build_dataset.dl
...
def _train_epoch (model, optimizer, loss_fn, data_loader, c_epoch, t_epoch, device, topk=2):
   ...
   # data = dataiter.next()
    #total=len(list(data_loader)), 
    with tqdm(total=len(list(data_loader)),ascii=True, desc='Epoch {}/{}: '.format(c_epoch, t_epoch), ncols=100) as t:
...
def train_model (model, train_data_loader, val_data_loader, optimizer=None, loss_fn=None, epochs=10,
                 epochs_early_stop=None, save_folder=("C:/Users/BOLD I.T/Desktop/jedyy/pytorch"), initial_model=None, best_metric="loss", device=None,
                 topk=2, schedule_lr=None, config_bot=None, model_name="MOBILENET CNN", resume_train=False, history_plot=True):
    """
